using UnityEngine;

public class Skill_Yshtolla : Skill
{
    [SerializeField]
    float explosionDuration = 0.5f;

    [SerializeField]
    float laserDuration = 0.6f;

    [SerializeField]
    GameObject laserPrefab;

    bool _passiveActive;

    void Update()
    {
        if (Input.GetKeyDown(KeyCode.J))
            Chain1(testPos, 1f);

        if (Input.GetKeyDown(KeyCode.K))
        {
            Chain1(testPos, 1.5f);
            Chain2(testPos, 1.5f);
        }

        if (Input.GetKeyDown(KeyCode.L))
        {
            Chain1(testPos, 2f);
            Chain2(testPos, 2f);
            Chain3(testPos, 2f);
        }
    }

    public override void Chain1(Vector3 targetPos, float scaleFactor) =>
        Attack(targetPos, scaleFactor);

    public override void Chain2(Vector3 targetPos, float scaleFactor) =>
        Attack(targetPos, scaleFactor);

    public override void Chain3(Vector3 targetPos, float scaleFactor) =>
        Attack(targetPos, scaleFactor);

    void Attack(Vector3 targetPos, float scaleFactor)
    {
        if (_passiveActive && laserPrefab != null)
        {
            var laser = Instantiate(laserPrefab, targetPos, Quaternion.identity);
            laser.transform.localScale = Vector3.one * scaleFactor;
            Destroy(laser, laserDuration);
        }
        else if (effectPrefab != null)
        {
            var go = Instantiate(effectPrefab, targetPos, Quaternion.identity);
            go.transform.localScale = Vector3.one * scaleFactor;
            Destroy(go, explosionDuration);
        }
    }

    public void SetPassiveActive(bool active)
    {
        _passiveActive = active;
    }
}
